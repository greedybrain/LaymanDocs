...
const handlingFormErrors = (layman) => {
     if (layman.errors.password.length > 0) {
          layman.errors.password.forEach(pswErr => {
               let pswErrMsg = document.createElement('p')
               pswErrMsg.textContent = pswErr
               pswErrMsg.style.color = "#fff"
               form.append(pswErrMsg)
          })
     }
}